...
def count
  puts "They are #{journalist.count} journalists."
end

def sort
  puts "The shortest handle is #{journalist.min_by(&:length)}."
end

def five_characters
  five_characters = journalist.select {|five| five.size == 6}.count
  puts "They are #{five_characters} handles with 5 characteres."
end

def uppercase
  uppercase = journalist.select {|result| result[0..1] =~ /[A-Z]/}.count
  puts "They are #{uppercase} handles start with uppercase."
end
...
def epenser
  puts "In this array, @epenser is position #{journalist.index "@epenser"}"
end
